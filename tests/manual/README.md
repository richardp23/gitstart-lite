...
These checks need a real interactive terminal. The automated suite does not
test `/dev/tty`, `stty`, or arrow-key input.

Do not add `expect`, `tmux`, `script`, or another PTY tool as a project
dependency only to automate these checks (D-017).

## Environment

Run each check in:

1. Current Git Bash on Windows
2. macOS system Bash 3.2 (Terminal) when available
...
| MT-013 | Parent navigation | FR-063 | Highlight `..` or press Left / Backspace to move up |
...
| MT-018 | Narrow terminal | NFR-022 | A narrow window remains usable; `p` toggles preview |
| MT-019 | One frame | D-009 | Arrow movement does not stack duplicate lists |
| MT-020 | Compact preview | FR-080–FR-085 | Wide terminals show a short preview; medium shows one line |

## Mode and display

| ID | Check | Requirement IDs | Pass condition |
|----|-------|-----------------|----------------|
| MT-030 | Learn mode typing | FR-050, FR-051 | Student must type the teaching command |
| MT-031 | Assisted confirm | FR-052, FR-053 | Student confirms instead of typing |
| MT-032 | `--no-color` | FR-024, NFR-021 | Status labels remain without color |
| MT-033 | `--ascii` | FR-026, FR-043 | ASCII banner appears |
| MT-034 | No alternate screen | FR-047, FR-048 | Prior terminal output remains visible |
...
```text
Manual terminal checks
- Git Bash (Windows): pass / fail / not run
- macOS Bash 3.2: pass / fail / not run
- Failed IDs:
- Notes:
```

Required before a classroom release: Git Bash on Windows and macOS system
Bash 3.2 when a Mac is available. Record the block above.
